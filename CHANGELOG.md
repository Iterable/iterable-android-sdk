...
### Fixed
- Push token registration, disable operations, and Iterable deep-link redirects now run on an SDK-owned serial executor instead of Android's process-wide `AsyncTask` queue, preventing host app background work from delaying SDK operations while preserving operation order. Deep-link callbacks and attribution updates continue on the main thread.
