...
### Fixed
- Push token registration and disable operations now run on a dedicated SDK-owned serial executor, while Iterable deep-link redirects use a separate SDK-owned serial executor. This removes their dependency on Android's process-wide `AsyncTask` queue, preserves ordering within each operation type, and prevents slow redirects from delaying push work. Deep-link callbacks and attribution updates continue on the main thread.
