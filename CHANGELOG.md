...
### Fixed
- Push token registration and disable operations now run on a dedicated SDK-owned serial executor instead of Android's process-wide `AsyncTask` queue, preventing host app background work from delaying device registration while preserving operation order.
