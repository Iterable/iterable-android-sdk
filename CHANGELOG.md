...
## [Unreleased]
### Added
- Added `IterableApi.switchProject(context, project, callback)` for apps that need to move a running install from one Iterable project to another (for example a multi-region app switching between a US and an EU project) without an app restart. The method returns immediately and performs the whole sequence on the SDK's background executor: it disables the push token on the previous project, clears that project's identity from memory and from storage, drops its cached in-app, embedded and unknown-user state along with its activation criteria and push attribution, purges the persisted offline queue, rebuilds the auth manager and the keychain against the new config, and re-initializes with the new API key. SDK calls made between the call and the callback are queued and then run in order against the new project.
  - The callback is a new `IterableProjectSwitchCallback`, delivered on the main thread. Its single method `onProjectSwitched(IterableProjectSwitchResult result)` reports either `SWITCHED_CLEANLY` or `SWITCHED_WITH_WARNINGS`. Both mean the SDK **is** on the new project; they differ only in whether leaving the previous one was tidy, and neither means the switch failed or was rolled back. A named result rather than a boolean because the meaning of a bare `false` had to be explained everywhere it appeared. `SWITCHED_WITH_WARNINGS` is expected in normal operation and is not an error: an app that does not use push, or that has no device token yet, always sees it, because the switch cannot confirm a device disable for the previous project. The response to both is the same, carry on and re-identify the user. It is a single-method interface so that a lambda receives the result: reusing `IterableInitializationCallback` would not work, because its only abstract method takes no arguments and a lambda would bind to that one and discard the result.
  ...
  - Added `IterableProject`, which pairs a project's API key with the `IterableConfig` to run it with. `switchProject` takes one of these rather than a loose key and config, because `IterableConfig` carries the data region and the `IterableAuthHandler`: supplying the two halves separately allows one project's key to be paired with another project's region or auth handler, and that mismatch produces an authentication failure, or a request sent to the wrong region, that looks unrelated to the project it came from. `IterableProject` also rejects a null or blank API key at construction with `IllegalArgumentException`, so an unusable project cannot be built and a blank key can no longer reach a teardown.
  - Called before any `initialize` the callback reports `SWITCHED_CLEANLY`, matching iOS. There is no previous project, so no teardown step could have been noisy and there was no device to disable. Note that `initializeInBackground` reports completion whether or not initialization succeeded, so on Android this result does not distinguish a failed initialization; iOS does.
  ...
  - The disable for the previous project captures that project's API key **and** its region endpoint when it is initiated, and the switch waits (up to 2 seconds) for the disable to be handed to the request layer before swapping. `users/disableDevice` is project-scoped on the backend, so without the captured key the disable would usually land on the new project instead, leaving the previous project still delivering push to the device; without the captured endpoint a switch between data regions would send the previous project's key to the new project's region, which is rejected. If the wait times out the switch still completes and the callback reports `SWITCHED_WITH_WARNINGS`, and the disable that dispatches afterwards still reaches the project it was created for.
  - `SWITCHED_CLEANLY` means every teardown step completed at the point the callback fired. It is not a guarantee that the device disable reached the network: the disable is handed to the request layer, which may queue it, and the callback is not held open for the response. This matches the iOS SDK. An app that needs certainty about the outgoing project's device state should not infer it from this callback.
  - Called before any `initialize`, it behaves as `initializeInBackground` and logs a warning. Called while an initialization is still in flight, it waits for that initialization and then switches.
  - Repeated calls are resolved against the project most recently *asked* for rather than the one currently live, so the SDK always ends up where the app last asked to be. A request for the project already asked for is absorbed: with nothing in flight it is a no-op reporting `SWITCHED_CLEANLY`, and while a switch to it is running its callback is registered with that switch rather than starting a second teardown, so a picker tapped twice on one destination runs a single teardown. Anything else is queued and runs in the order it was asked for, each callback firing when the project it asked for is live. Asking to go back to the project a running switch is leaving is therefore a real switch and not a no-op, and `A → B → A` reports B and then A and ends on A. Between two switches in a chain the SDK is fully live on the project that just landed, so calls made from its callback, re-identifying the user in particular, run against that project instead of being replayed into the next one.
...
- `switchProject` called for a second project while a switch is still running no longer discards that request. The SDK used to finish on the first project while the second caller's callback still reported a completed switch, so an app whose region or brand picker was tapped twice ended up sending every later event to the project the user had already moved away from, with nothing in the API to detect it. The second request is now run as soon as the first finishes, on the chain the first hands to it rather than a fresh one, so a third request made in between, from the first switch's callback for instance, cannot claim the chain first and then be overtaken by the queued one. Requests are resolved against the project asked for most recently, so repeated taps on one destination still share a single teardown, a repeat that crosses another destination runs again in its turn rather than being folded into the earlier one, and asking to go back to the project a running switch is leaving is honoured instead of being mistaken for a no-op and reported as a clean switch that never happened.
- Calls queued during a project switch can no longer be overtaken by a call that arrives as the switch lands. The gate was lowered before the queue was drained, so a `setEmail` made at that moment could run first and then be overwritten by the older one replayed behind it. The gate now stays raised until the queue has been drained.
- Identity set from a `switchProject` callback now reaches the project that just landed, even when another switch is already queued behind it. The queued switch inherited a still-raised gate, so a `setEmail` made from the callback, which is where the contract says to make it, was queued and then replayed against the next project in the chain, identifying one project's user on another. The gate that queues calls is now released as each switch lands, and the ordering of queued switch requests is tracked separately from it.
- Attribution resolved by a deep link redirect that was started on the previous project is no longer written into the new project's storage. The redirect is a network round trip, so a switch can land in the middle of one, and the campaignId, templateId and messageId it returns exist only in the project that was left. The redirect now carries the API key that was live when the link was clicked and its result is discarded if that project is no longer the active one. iOS already guarded this.
