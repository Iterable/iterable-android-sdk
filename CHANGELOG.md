...
### Fixed
- Fixed a regression introduced in 3.6.5 where a custom action remained pending when `IterableCustomActionHandler` returned `false`, causing the same action to run again on later foreground and initialization events. A custom action is now consumed once its handler is invoked, while existing retry behavior for actions that were not dispatched remains unchanged.
